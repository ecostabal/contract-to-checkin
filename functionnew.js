...
async function processSubElementsAndCreateItems(boardId, itemId) {
    try {
        ...
        for (const subitem of subitemsData) {
            ...
            if (signerTypeColumn && signerTypeColumn.text === 'Arrendador') {
                if (Object.keys(arrendadorData).length === 0) {
                    arrendadorData = {
                ...
                continue; // Saltar al siguiente subitem
            }

            if (signerTypeColumn && signerTypeColumn.text === 'Arrendatario') {
                // Procesar datos del arrendatario
                ...
                // Crear nuevo ítem en el otro tablero
